...
def show_vid(name,func):
    ...
    output = cv2.VideoWriter('out_edge.mp4',cv2.VideoWriter_fourcc(*'mp4v'), 25, frame_size,isColor=False)
...
def abs_sobel_(img,min_thresh=25,max_thresh=255,sobel_kernel = 3):
    ...
    l_channel = hls[:,:,1]
    s_channel = hls[:,:,2]
    abs_sobel_h_channel = np.absolute(cv2.Sobel(h_channel, cv2.CV_64F, 1, 1,ksize=sobel_kernel))
    abs_sobel_s_channel = np.absolute(cv2.Sobel(s_channel, cv2.CV_64F, 1, 1,ksize=sobel_kernel))
    abs_sobel_l_channel = np.absolute(cv2.Sobel(l_channel, cv2.CV_64F, 1, 1,ksize=sobel_kernel))

    scaled_sobel_s = np.uint8(255*abs_sobel_s_channel/np.max(abs_sobel_s_channel))
    binary_output_s = np.zeros_like(scaled_sobel_s)
    binary_output_s[(scaled_sobel_s >= min_thresh) & (scaled_sobel_s <= max_thresh)] = 1

    scaled_sobel_l = np.uint8(255*abs_sobel_l_channel/np.max(abs_sobel_l_channel))
    binary_output_l = np.zeros_like(scaled_sobel_l)
    binary_output_l[(scaled_sobel_l >= min_thresh) & (scaled_sobel_l <= max_thresh)] = 1

    scaled_sobel_h = np.uint8(255*abs_sobel_h_channel/np.max(abs_sobel_h_channel))
    binary_output_h = np.zeros_like(scaled_sobel_h)
    binary_output_h[(scaled_sobel_h >= min_thresh) & (scaled_sobel_h <= max_thresh)] = 1
...
def edge_detection(img,min_thresh=10,max_thresh=255,dx=300,dy=200):
    ...
    edge_s,edge_l,edge_h = abs_sobel_(blur,min_thresh=min_thresh,max_thresh=max_thresh)
...
def filter_image(img):
    ...
    dir_binary = dir_threshold(img, sobel_kernel=5, thresh=(45*np.pi/180, 75*np.pi/180)) 
