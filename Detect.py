...
import numpy as np
from util import radius_of_curvature, draw_poly
...
class Detector():
    ...
    def detect_lanes(self, b_img, org_img, win_width=100):

        out = np.dstack(
            (np.zeros_like(b_img), np.zeros_like(b_img), np.zeros_like(b_img)))
        width = b_img.shape[1]
        ...
        # minlanepts missing
        # return tuple of 2 array contains x and y indxs to pixels with nonzero value
        whitepxls = b_img.nonzero()
        ...
        if self.detect:
            ...
            for window in range(n_windows, 0, -1):
                ...
                pixel_indxs_in_y = (whitepxls_y >= lower_y_window) & (
                    whitepxls_y <= upper_y_window)
                left_pxls_indxs = (whitepxls_x >= lower_left_x) & (
                    whitepxls_x <= upper_left_x) & pixel_indxs_in_y
                right_pxls_indxs = (whitepxls_x >= lower_right_x) & (
                    whitepxls_x <= upper_right_x) & pixel_indxs_in_y
    ...
    def visualize(self, org_img, b_img, poly_left, poly_right, left_line_X, right_line_X):
        plt_y_pts = np.linspace(
            0, b_img.shape[0]-1, b_img.shape[0])
        rid_cur, car_offset = radius_of_curvature(
            b_img, plt_y_pts, left_line_X, right_line_X)
        result = draw_poly(org_img, b_img, poly_left,
                           poly_right, rid_cur, car_offset)
        return result
